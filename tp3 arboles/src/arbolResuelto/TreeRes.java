...
public class TreeRes {
	...
	public void add(Integer value) {
		if (this.root == null)
			this.root = new TreeNodeRes(value);
		else
			this.add(this.root,value);
	}
	private void add(TreeNodeRes actual, Integer value) {
		if (actual.getValue() > value) {
			if (actual.getLeft() == null) { 
				TreeNodeRes temp = new TreeNodeRes(value);
				actual.setLeft(temp);
			} else {
	 ...
	 */
	public int getRoot() {
		TreeNodeRes nuevoRoot = this.root;
	...
	public void printInorder(TreeNodeRes node) 
    { 
        if (node == null) 
            return; 
	...
	private void printPosOrder(TreeNodeRes node) {
        if (node == null) 
            return; 
	...
	private void printPreOrder(TreeNodeRes node) {
        if (node == null) 
            return; 
	 ...
	 *
	 */
	private int getHeight(TreeNodeRes node) {
		int heightIzq=0;
		int heightDere=0;
	...
	public boolean Delete(TreeNodeRes node, int value) {
		if(node==null)return false;
		if(node.getLeft()!=null) {
			TreeNodeRes hijoIzq = node.getLeft();
			if(hijoIzq.esHoja()) {
		...
		if(node.getRight()!=null) {
			TreeNodeRes hijoDer = node.getRight();
			if(hijoDer.esHoja()) {
	...
	private List<Integer> getLongestBranch(TreeNodeRes node) {
	...
	private List<Integer> getFrontera(TreeNodeRes node) {
		if(node==null)return new ArrayList<>();
	...
	private int getMaxElem(TreeNodeRes node) {
		int valor=0;
		if(node==null) return -1;
			if(node.getRight()!=null) {
	...
	private int getSumaNodos(TreeNodeRes node) {
	...
	private List<Integer> valoresSuperioresA(TreeNodeRes node, int value) {
	...
	public void printTree() {
        ...
        List<TreeNodeRes> level = new ArrayList<>();
        List<TreeNodeRes> next = new ArrayList<>();
	...
	public void addCaracter(char caracter) {
	    if (this.root == null) {
	        this.root = new TreeNodeRes(caracter);
	    } else {
	        addCaracterRec(this.root, caracter);
	    }
	}

	private void addCaracterRec(TreeNodeRes actual, char caracter) {
	    if (actual == null) return;
	    if (actual.getLeft() == null) {
	        actual.setLeft(new TreeNodeRes(caracter));
	        return;
	    }
	    if (actual.getRight() == null) {
	        actual.setRight(new TreeNodeRes(caracter));
	        return;
	    }
	...
	private List<Character> buscarVocales(TreeNodeRes actual, int valor, List<Character> todosVal) {
